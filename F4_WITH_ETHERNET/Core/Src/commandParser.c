...
uint8_t runCommand(commandTemplate* current_command){
    ...
    //I2C_EXPANDER <ACTION> [...]
        ...
        if (strcmp(current_command->subcommands[0],"SELECT_CHANNEL") == 0){
            ...
            if (result) {
                ...
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;I2C_EXPANDER SELECT_CHANNEL FAILED HAL_ERROR:%lu\n", pca9548apwr_expander.last_i2c_error);
            }
        }
        ...
        //I2C_EXPANDER GET_SELECTED_CHANNEL
            ...
            if (selected_channel != PCA9548APWR_NO_CHANNEL_SELECTED) {
                snprintf(current_command->response, sizeof(current_command->response), "OK;I2C_EXPANDER SELECTED_CHANNEL:%lu\n", selected_channel);
                ...
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;I2C_EXPANDER GET_SELECTED_CHANNEL FAILED HAL_ERROR:%lu\n", pca9548apwr_expander.last_i2c_error);
            }
        }
        ...
        //I2C_EXPANDER INIT
            ...
            if (result) {
                ...
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;I2C_EXPANDER INITIALIZATION FAILED HAL_ERROR:%lu\n", pca9548apwr_expander.last_i2c_error);
            }
        }
        ...
        //I2C_EXPANDER WRITE_CONTROL_REGISTER [VALUE]
            ...
            if (result) {
                ...
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;I2C_EXPANDER WRITE_CONTROL_REGISTER FAILED HAL_ERROR:%lu\n", pca9548apwr_expander.last_i2c_error);
            }
        }
         ...
         //I2C_EXPANDER READ_CONTROL_REGISTER
            ...
            if (register_value >= 0) {
                ...
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;I2C_EXPANDER READ_CONTROL_REGISTER FAILED HAL_ERROR:%lu\n", pca9548apwr_expander.last_i2c_error);
            }
        }
        ...
        //I2C_EXPANDER SET_SPEED [VALUE IN HZ]
            ...
            snprintf(current_command->response, sizeof(current_command->response), "OK;I2C_EXPANDER SET_SPEED:%lu\n", clock_speed);
        }
    ...
    //I2C [1-3] REINIT [SPEED IN HZ]
        ...
        if (strcmp(current_command->subcommands[1],"REINIT") == 0){
            ...
            snprintf(current_command->response, sizeof(current_command->response), "OK;I2C %u REINIT SPEED_HZ:%lu\n", bus_index, speed_hz);
    ...
    //TOF [0-7] <ACTION> [...]
        ...
        if (strcmp(ACTION, "SET_I2C_SPEED") == 0){
            ...
            osDelay(20);
            snprintf(current_command->response, sizeof(current_command->response), "OK;TOF %u SET_I2C_SPEED:%lu\n", sensor_index, speed_hz);
            osMutexRelease(take_measurement_mutex);
            return 1;
        }
        ...
        if (PCA9548APWR_getSelectedChannel(&pca9548apwr_expander) != sensor_index){
            ...
            {
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;TOF %u I2C expander channel is not set to the TOF sensor and failed to set it correctly HAL_ERROR:%lu\n", sensor_index, pca9548apwr_expander.last_i2c_error);
                osMutexRelease(take_measurement_mutex);
                return 0;
            }  
        }
        ...
        if (strcmp(ACTION, "UNSTUCK_STALLED_I2C_BUS") == 0){
            ...
            if (recovery_result != 0){
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;TOF %u UNSTUCK_STALLED_I2C_BUS failed with error:%li\n", sensor_index, recovery_result);
                osMutexRelease(take_measurement_mutex);
                return 0;
            }
        ...
        //TOF [0-7] INIT
            ...
            if (sensor_staus_result != 0){
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;TOF %u INITSENSOR FAILED with error:%li\n", sensor_index, sensor_staus_result);
                osMutexRelease(take_measurement_mutex);
                return 0;
            }
        ...
        //TOF [0-7] SET_MODE [SHORT/LONG/1/2]
            ...
            if (sensor_staus_result != 0){
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;TOF %u SET_MODE FAILED with error:%li\n", sensor_index, sensor_staus_result);
                osMutexRelease(take_measurement_mutex);
                return 0;
            }
        ...
        //TOF [0-7] GET_BOOT_STATE
            ...
            if (sensor_staus_result != 0){
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;TOF %u GET_BOOT_STATE FAILED with error:%li\n", sensor_index, sensor_staus_result);
                osMutexRelease(take_measurement_mutex);
                return 0;
            }
        ...
        //TOF [0-7] SET_INTER_MEASUREMENT_PERIOD [PERIOD_MS]
            ...
            if (sensor_staus_result != 0){
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;TOF %u SET_INTER_MEASUREMENT_DELAY FAILED with error:%li\n", sensor_index, sensor_staus_result);
                osMutexRelease(take_measurement_mutex);
                return 0;
            }
            snprintf(current_command->response, sizeof(current_command->response), "OK;TOF %u SET_INTER_MEASUREMENT_DELAY:%lu ms\n", sensor_index, period_ms);
            osMutexRelease(take_measurement_mutex);
            return 1;
        }
        ...
        //TOF [0-7] SET_TIMING_BUDGET [BUDGET_MS]
            ...
            if (sensor_staus_result != 0){
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;TOF %u SET_TIMING_BUDGET FAILED with error:%li\n", sensor_index, sensor_staus_result);
                osMutexRelease(take_measurement_mutex);
                return 0;
            }
            snprintf(current_command->response, sizeof(current_command->response), "OK;TOF %u SET_TIMING_BUDGET:%lu ms\n", sensor_index, budget_ms);
            osMutexRelease(take_measurement_mutex);
            return 1;
        }
        ...
        //TOF [0-7] SET_ROI [X] [Y]
            ...
            if (sensor_staus_result != 0){
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;TOF %u SET_ROI FAILED with error:%li\n", sensor_index, sensor_staus_result);
                osMutexRelease(take_measurement_mutex);
                return 0;
            }
        ...
        //TOF [0-7] SET_DISTANCE_TRESHOLD [MIN_MM] [MAX_MM] [0:bellow|1:above|2:out|3:in] [IntOnNoTarget]
            ...
            if (sensor_staus_result != 0){
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;TOF %u SET_DISTANCE_TRESHOLD FAILED with error:%li\n", sensor_index, sensor_staus_result);
                osMutexRelease(take_measurement_mutex);
                return 0;
            }
        ...
        //TOF [0-7] CLEAR_INNTERUPT
            ...
            if (sensor_staus_result != 0){
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;TOF %u CLEAR_INTERRUPT FAILED with error:%li\n", sensor_index, sensor_staus_result);
                osMutexRelease(take_measurement_mutex);
                return 0;
            }
        ...
        //TOF [0-7] MEASURE_DISTANCE [TIMEOUT_MS]
            ...
            if (sensor_staus_result != 0){
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;TOF %u MEASURE_DISTANCE FAILED with error:%li\n", sensor_index, sensor_staus_result);
                osMutexRelease(take_measurement_mutex);
                return 0;
            }
        ...
        //TOF [0-7] DO_LIMIT_CALIBRATION [TIME_OF_CALIBRATION_MS] [FINAL_LIMIT_OFFSET_PERCENT]
            ...
            if (calibration_result != 0){
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;TOF %u DO_LIMIT_CALIBRATION failed with error:%li\n", sensor_index, calibration_result);
                osMutexRelease(take_measurement_mutex); //Release mutex before returning
                return 0;
            }

            snprintf(current_command->response, sizeof(current_command->response), "OK;TOF %u DO_LIMIT_CALIBRATION successful, min_limit:%li, max_limit:%li\n", 
            sensor_index, 
        ...
        //TOF [0-7] CHECK_IF_MATCHES_WINDOW [DISTANCE_MM]
            ...
            if (sensor_staus_result != 0){
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;TOF %u CHECK_IF_MATCHES_WINDOW failed with error:%li\n", sensor_index, sensor_staus_result);
                osMutexRelease(take_measurement_mutex);
                return 0;
            }
        ...
        //TOF [0-7] CHECK_INTERRUPT
            ...
            if (sensor_staus_result != 0){
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;TOF %u CHECK_INTERRUPT failed with error:%li\n", sensor_index, sensor_staus_result);
                osMutexRelease(take_measurement_mutex);
                return 0;
            }
        ...
        //TOF [0-7] GET_INFO
            ...
            if (sensor_staus_result != 0){
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;TOF %u GET_INFO failed with error:%li\n", sensor_index, sensor_staus_result);
                osMutexRelease(take_measurement_mutex);
                return 0;
            }

            snprintf(current_command->response, sizeof(current_command->response), "OK;TOF %u INFO BOOT_STATE:%d MIN_LIMIT:%u mm MAX_LIMIT:%u mm DIST_THRESHOLD_MIN:%u mm DIST_THRESHOLD_MAX:%u mm DIST_THRESHOLD_WINDOW:%u mm TIMING_BUDGET:%u ms INTER_MEASUREMENT_DELAY:%lu ms DISTANCE_MODE:%u\n",
        ...
        //TOF [0-7] RAW_I2C_READ_BYTE [REGISTER_ADDRESS]
            ...
            if (sensor_staus_result != 0){
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;TOF %u RAW_I2C_READ_BYTE failed with error:%li\n", sensor_index, sensor_staus_result);
                osMutexRelease(take_measurement_mutex);
                return 0;
            }
        ...
        //TOF [0-7] RAW_I2C_READ_WORD [REGISTER_ADDRESS]
            ...
            if (sensor_staus_result != 0){
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;TOF %u RAW_I2C_READ_WORD failed with error:%li\n", sensor_index, sensor_staus_result);
                osMutexRelease(take_measurement_mutex);
                return 0;
            }
        ...
        //TOF [0-7] RAW_I2C_WRITE_BYTE [REGISTER_ADDRESS] [VALUE]
            ...
            if (sensor_staus_result != 0){
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;TOF %u RAW_I2C_WRITE_BYTE failed with error:%li\n", sensor_index, sensor_staus_result);
                osMutexRelease(take_measurement_mutex);
                return 0;
            }
        ...
        //TOF [0-7] RAW_I2C_WRITE_WORD [REGISTER_ADDRESS] [VALUE]
            ...
            if (sensor_staus_result != 0){
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;TOF %u RAW_I2C_WRITE_WORD failed with error:%li\n", sensor_index, sensor_staus_result);
                osMutexRelease(take_measurement_mutex);
                return 0;
            }
        ...
        //TOF [0-7] SET_100HZ_MODE
            ...
            if (sensor_staus_result != 0){
                snprintf(current_command->response, sizeof(current_command->response), "ERROR;TOF %u SET_100HZ_MODE failed with error:%li\n", sensor_index, sensor_staus_result);
                osMutexRelease(take_measurement_mutex);
                return 0;
            }
    ...
    //EXT_32_RELAY_CARD [RELAY|PORT] [...]
        ...
        if (strcmp(current_command->subcommands[0],"RELAY") == 0){
            ...
            snprintf(current_command->response, sizeof(current_command->response), "OK;EXT_32_RELAY_CARD NEWPORTVALUE:%d, RELAY:%lu SET TO VALUE:%d\n", relay_number, EXTERNAL_32_RELAY_CARD.current_satus, relay_state);
            return 0;
        }
        ...
        if (strcmp(current_command->subcommands[0],"PORT") == 0){
            ...
            snprintf(current_command->response, sizeof(current_command->response), "OK;EXT_32_RELAY_CARD SET TO PORT VALUE:%lu\n", EXTERNAL_32_RELAY_CARD.current_satus);
            return 0;
        }
        else{
            snprintf(current_command->response, sizeof(current_command->response), "ERROR;No such subcommand\n");
    ...
    //DPDT_RELAYS [SET_PORT|SET_RELAY] [...]
        ...
        if (strcmp(current_command->subcommands[0],"SET_PORT") == 0){
            ...
            snprintf(current_command->response, sizeof(current_command->response), "OK;DPDT_RELAYS SET_PORT:%lu\n", port_value);    
        }
    ...
    //VENTILS [SET_PORT|SET_VENTIL] [...]
        ...
        if (strcmp(current_command->subcommands[0],"SET_PORT") == 0){
            ...
            snprintf(current_command->response, sizeof(current_command->response), "OK;VENTILS SET_PORT:%lu\n", port_value);    
        }
    ...
    //INPUTS [READ_ALL|READ_INPUT] [...]
        ...
        if (strcmp(current_command->subcommands[0],"READ_ALL") == 0){
            ...
            snprintf(current_command->response, sizeof(current_command->response), "OK;INPUTS READ PORT VALUE:%lu\n", (uint32_t)port_value);    
        }
    ...
    else if (strcmp(current_command->command,"ADC2_IN3?") == 0)
    {
        ...
        snprintf(current_command->response, sizeof(current_command->response), "OK;ADC2_IN3:%lu,Voltage:%lu mV\n", filtered_value / nr_of_samples, voltage);
    }
    ...
    else if (strcmp(current_command->command,"ADC2_IN6?") == 0)
    {
        ...
        uint32_t voltage = (filtered_value / nr_of_samples) * reference_voltage_mv / adc_max_value * voltage_divider;
        snprintf(current_command->response, sizeof(current_command->response), "OK;ADC2_IN6:%lu,Voltage:%lu mV\n", filtered_value / nr_of_samples, voltage);
    }
