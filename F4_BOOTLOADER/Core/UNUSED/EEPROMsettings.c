...
int8_t EEPROMgetDefaultMacAddress(uint8_t macAddress[6]){
    ...
    static uint8_t EUI64_ADDR[8];
    i2c_status =  HAL_I2C_Mem_Read(&hi2c2, EEPROM_I2C_ADDR_FOR_MAC, EEPROM_I2C_STARTADDR_OF_MAC, I2C_MEMADD_SIZE_8BIT, EUI64_ADDR, EEPROM_I2C_MAC_ADDR_LEN, HAL_MAX_DELAY);
    macAddress[0] = EUI64_ADDR[0];
    macAddress[1] = EUI64_ADDR[1];
    macAddress[2] = EUI64_ADDR[2];
    macAddress[3] = EUI64_ADDR[5];
    macAddress[4] = EUI64_ADDR[6];
    macAddress[5] = EUI64_ADDR[7];
    if(i2c_status != HAL_OK){ 
